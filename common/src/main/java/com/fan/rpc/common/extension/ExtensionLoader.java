...
import com.fan.rpc.common.extension.ann.*;
import com.fan.rpc.common.extension.context.Lifecycle;
import com.fan.rpc.common.utils.*;
...
public class ExtensionLoader<T> {
    ...
    public T getExtension(String name, boolean wrap) {
        ...
        // getOrCreateHolder() 方法中封装了 cachedInstances 缓存的逻辑
        // Holder 用于持有目标对象
        final Holder<Object> holder = getOrCreateHolder(name);
        Object instance = holder.get();
        // double - check  检测防止并发问题
        if (instance == null) {
            synchronized (holder) {
                instance = holder.get();
                if (instance == null) {
                    // 根据扩展名从 SPI 配置中查找对应的扩展实现类
...
//        sort(instances, Prioritized.COMPARATOR);
    ...
    private T createExtension(String name, boolean wrap) {
        // 获取 cachedClasses 缓存，根据扩展名从 cachedClasses 中获取扩展实现类
...
//                    wrapperClassesList.sort(WrapperComparator.COMPARATOR);
    ...
    private T injectExtension(T instance) {
        ...
        try {
            ...
            for (Method method : instance.getClass().getMethods()) {
                // 判断方法是否以 set 开头，且方法仅有一个参数，且方法的访问级别为 public
                if (!isSetter(method)) {
                    continue;
                }
                /**
                ...
                try {
                    // 获取属性名称，比如 setName 方法对应的属性名 name
    ...
    private Map<String, Class<?>> getExtensionClasses() {
        ...
        if (classes == null) {
            // 如果 cachedClasses 未被初始化
            synchronized (cachedClasses) {
                classes = cachedClasses.get();
                if (classes == null) {
                    // 加载扩展文件
                    classes = loadExtensionClasses();
                    // 扩展名和扩展实现类的映射关系记录到 cachedClasses 缓存中
...
//            loadDirectory(extensionClasses, strategy.directory(), type.getName().replace("org.apache", "com.alibaba"), strategy.preferExtensionClassLoader(), strategy.overridden(), strategy.excludedPackages());
    ...
    private void cacheDefaultExtensionName() {
        // 获取 SPI 注解，这里的 type 变量在调用 getExtensionLoader 方法时传入的
